...
## 2026-07-22~24 — 콜리그 retrieval 검증 → R-시리즈 → **W-A 확증 최종(+6.9pp SIG)** → 논문 v2 → 미래 셀 사전등록, 상세 = `reports/2026-07-22_retrieval_capacity_session/`
- **★HEADLINE — W-A 확증 최종**: 3 train-seed × 2 arm(matchedbase/wristpatch) × 2 mode × 500ep = **6,000ep**, per-episode JSONL 직접 재계산(유실 verdict 스크립트 대체 = 3f0d984 provenance 하네스 자기입증). **matchedbase 85.7 / wristpatch 92.6 = +6.9pp, paired bootstrap CI[+4.9,+9.1] = SIG>0** — **캠페인 최초의 확증된 양의 SR 아키텍처 추가**(h-flow/actionflow/crop/W-C 전부 중립~음성이던 것의 유일 예외 = 조건화-측 wrist 국소 기하). per-task 전부 양·파지/재참조 집중(t5+11/t9+13), 3-seed 일관(+8.2/+5.6/+7.0), 스크리닝 +5.5와 정합. **BUT 언어 공동기준 wristpatch +63.7 vs base +73.9** — 유보밴드(65–75) 하단도 하회(wrong 지시에서도 28–30% 성공 = 손목 기하로 언어 없이 파지). ∴ 사전등록 규칙(SR CI>0 AND c−w≥+70) 미충족 → **"채택 아키텍처" 승격 안 함, SR↔언어 tradeoff 프런티어의 새 점**. **SR↔언어 법칙의 4번째 독립 재현**(융합·crop·W-A 스크리닝에 이어) = C-2 기여 결정적 강화. 프런티어 확정 점 2개 추가(85.7/+73.9, 92.6/+63.7).
- **콜리그 retrieval 제어 검증(READ-ONLY 크로스랩)**: 동료 "retrieval=wow"의 기전 = **벡터 주입이 아니라 언어→검색→시연 프리미티브 오픈루프 재생**(정책·h·Δz 미경유). **랩 내 이중 해리** — 같은 어댑터로 주입(E5 잔차 0.58 FAIL)/검색(8/8) 갈림 = 우리 WEEK1(T-0 사망·A5 통과) 교차랩 재현. **C3-강(zero-demo 벡터산술) 사망 보강 / C3-약(검색-매개 선택) 부활**. 정직 한계 그들 JSON 자체 기록: 실행 n=1·camera-frame 0/4·8클래스 단일객체.
- **R-시리즈(어댑터 우리 기질 이식, CPU — GPU는 W-A 확증 점유)**: 분할 동료-정확 일치(1995/497 seg). R-0 canonical **0.972**(그들 0.974) PASS, unseen 0.952 PASS, **우리 독립 3rd셋 0.773 G-R0c-2 FAIL = paraphrase 불변성의 어휘-반경 한계**(grasp 붕괴, 학습동사 밖 SigLIP2 텍스트기하 임의이웃). 정직대조: 텍스트-무관 MLP 0.968≈어댑터(분류기 등가), 상태-잔차화 0.749 급락. R-1 이중해리 재현(correct 8/8·스왑 56/56·셔플 마진 1/3 붕괴·넌센스 0.06). **R-0b: canonical 판별력 70% 상태-무관 / 일반화 ~55% 상태-운반**, 그리퍼 이벤트 state-free·approach/place 상태-의존(클래스 비대칭이 핵심). C3′ = 혼합 대역으로 정직 한정.
- **논문 아키텍처 v2("SR이 아니라 기여")**: 3 법칙급 기여 확정 — C-1(구조적 언어 상속, 개입 0으로 Biased 2.5% vs VLA 45–79%), C-2(삽입점 지도 **+ SR↔언어 tradeoff LAW를 별도기여 아닌 조직 프레임으로 통합**, 다이얼 3회 재현), C-3=C3′(주입 사망/검색 생존 이중해리 + 상태-지분·어휘반경 정직 한계). 시그니처 = F1 프런티어 그림(신규런 0). 얼굴 = **해석-루프 데모**(행동 前 의도 판독). 실험 큐 E1–E9, LIBERO-CF 공인좌표 변환 계획.
- **CPU 사전게이트 3종**(GPU 점유 중): **G-D1 데모 FAIL** — 정책 ζ̂ top-1 0.363(데이터 ζ와 cos 0.188, 학습시 lat_cos 0.203과 정합 = 버그 아닌 분포이동), ζ̂-재적합 후 0.786<0.85 → **E2 데모 "오프라인 판독 그림"으로 강등**(마진게이팅 0.856@80%·place∪release 병합 0.871 부분구제). **R-D0 W-D GO 아슬아슬**(r_full−r_state +0.0227≥+0.02, 마진 0.0027 — 초과신호 거의 a_emb). **R-A′ W-A′ GO 동등**(SigLIP2 wrist uplift +0.0459 ≈ DINO +0.0456, 비율 1.008 → "W-A 이득=DINO 기하 특이" 오프라인 기각, 손목 패치정보 인코더-불문).
- **launch-ready 큐(GPU 창 대기)**: P-B **LangSelPool(B1)** 구현(093cc1b, 텍스트-쿼리 patch pooling, F3 결함 전면 수정, 롤아웃 언어 인과 유지) + **g/h 용량 스윕** 사전등록(config 8종·probe_h_jacobian, PI "phase1 용량 부족?" 의심을 오프라인 4–8 GPU-h로 영구 종결하는 게이트) + **W-D/W-A′** 사전등록(팔당 변경 하나·guarded byte-identity). 전부 착수 대기, 킬게이트로 지출 캡.

## 2026-07-18~21 — 신뢰성 감사 → 재설계 → wrist 캠페인: **W-A 첫 양성(+5.5pp)**, 상세 = `reports/2026-07-18_wrist_fusion_session/`
- **감사(3부작)**: ① 콜리그 flow 크로스랩 — "actionflow 97.2"는 콜리그 스스로 3-seed 96.2≈base 강등, decoder-측 h-flow 양성은 원래 없었음(우리 33/37%는 무모순); 우리 실결함 = quantile actnorm 미이식 + `--flow-fixed-noise` 모드락 아님(수정: `--flow-noise-mode locked`). ② 음성결론 감사 — C1/C2 게이트 **∂L_act/∂α≡0 구조 결함**(일반화 철회), F3 naive 모듈+120ep confound, wrist "baseline 하회" 통계 미성립(재분류: 미완). ③ provenance — 헤드라인 10개 로컬 UNTRACED → 원격 로그 회수로 7 EXACT 재계산 + 게이트 α 실측 7자리 일치(날조 아님 확정). 하네스에 per-episode JSONL 상시 기록 추가(재발 방지).
- **오프라인 프로브(week0/1, 전부 CPU)**: P-zg **RED** — align의 ~2/3가 상태 성분, 상태단독 ridge가 g를 상회(→ innovation-grounding 잔차화 발동 대기); h eff-rank ~5 재현. **발산 포트폴리오 10개 중 언어군 전멸**(T-0: Δz_text 주입≈셔플 무우위, 개념기저/검색/심판자 KILL) — 사망 원인 = 태스크-수준 텍스트↔청크-수준 변위의 방향 대응 부재+시간입도 불일치. **생존 자산**: modality gap이 Δ에서 상쇄(0.277) 실증 = Δ-접지의 이론 방어선; "언어=태스크선택 수준, 청크벡터 의미교환 부재" 천장 지도 완성.
- **P-A center-crop 셀 종결(음성)**: 3팔 20롤 스크리닝 전부 G-cl 탈락(avg −0.5/concat **−3.5 유의 하락**/both +1.5 NS) — "+5pp 전처리 레버" 철회. wrong SR 상승 = 시각정보↑→언어의존↓ tradeoff 재확인.
- **wrist 캠페인(W1-W5 파이프라인: 문헌→브리프→설계→적대검증→수정)**: matched baseline **최초 실행**(87.0, 재현 정상 — 종전 비교는 대조군 경로버그로 0회 실행이었음 발견). 4팔 판정: **W-A(DINOv3 wrist 패치 2×2→4토큰, 조건화-측, phase1 불변) 92.5 = +5.5pp, 이득이 파지태스크 집중(t4 +35pp)** — **캠페인 최초 양성 아키텍처 추가**, 언어 +65.5 유보밴드로 확증 진출. W-B(변위 입력토큰) 91.5 널·W-A 하위호환. **W-C(변위 추론, 스케일 표준화 수리판) 82.5 널 — 표준화는 작동(align 균형 수렴)했으나 폐루프 무이득 = 타깃-측 wrist 추론 영구 종결**. 기전: wrist는 그리퍼 채널의 **상시** 우월 관측(R² 0.881 vs 0.725, 파지 집중 1.2배뿐). PI 제안 2건(스트림 분리 정책/동적 가중)은 측정 선행으로 기각 — 교차쌍 h 민감도 0.662(JOINT_REQUIRED), oracle 가중 headroom +0.000025(무익; N3 표준화가 이미 자체 가중 수행).
- **진행 중**: W-A 확증(50롤×3시드×2모드, ~6000ep) — 관건은 언어 공동기준 +70 회복.

---

## 2026-07-16 — Phase-B wrist-추론(dual-stream) 완료: 이득 없음 (전체 계획 완주)
- **wrist-cam을 추론 스트림으로 승격** (사용자 요청): `DualDeltaAE` — main=SigLIP2-large256 + wrist=DINOv3-CLS **별도 인코더**, g_main·g_wrist 분리 변위추론, 확장 h가 [ζ_main;ζ_wrist] 디코드. isolation base로 효과 격리.
- **offline 정상**: act R² +0.663, align_main·align_wrist 둘 다 수렴(스케일 불균형이 학습 안 깨뜨림), dual 롤아웃 경로 정상.
- **closed-loop: uplift 없음** — correct-mode 부분판독 3회(66.7/76.2/80.6%) 모두 large256-single baseline 85-88 하회. **Phase-A 복잡화-무익 패턴과 정합**(h-flow/actionflow/residual-flow/grid-token에 이어 dual-stream도 음성). wrist는 조건입력이 최선.
- ⚠ 롤아웃 CAP4/2/1 모두 27-84ep서 proc-death(osmesa per-process 누수 추정) → full 완주 불가·부분판독. 방향 결론은 견고, 정밀수치는 안정화 후 재검.
- **전체 계획 완주**(Phase-A 아키텍처 서치 + Phase-B wrist). 통합 결론: 값은 단순 관측융합 삽입점+전처리, 아키텍처 복잡화는 일관되게 무익.
...
### 2026-07-16 · DINOv3 사용 감사(정정) + h-flow 폐루프 실패·v2 수정 + DINOv2-avg 97.5(전처리 confound)
- **DINOv3 감사(사용자 우려 "잘못 불러왔나")**: `_dino_pooled`=`pooler_output`=post-LN CLS=`last_hidden[:,0]` = **DINOv3 논문/모델카드 권장 global(CLS) 정확** — register 4개는 idx1-4, idx0=CLS, **버그 없음**. 문헌상 DINOv3 ViT-L ImageNet linear-probe 87.44%≥DINOv2(~86%) → "DINOv3 global 약함"은 문헌 반대(희생은 dense, Gram anchoring이 복구·global 유지). **∴ 91.5(DINOv3) vs 97.5(DINOv2)는 백본 실차 아닌 전처리 confound**: DINOv2 arm=콜리그-exact **256→224 center-crop**, DINOv3 arm=**256 no-crop full-frame**. → 이전 "DINOv2>DINOv3, 업그레이드 손해" 판단 **철회(성급)**. 사용자 우려 정확. **정정 실험**: 매칭전처리(DINOv2@256-no-crop) + DINOv3@512(해상도 레버) 예정.
- **S2 h-flow 폐루프**: naive(fresh-noise 재계획) **33%**(concat-MLP 97.5 대비 급락) — 실패=**300스텝 timeout/wandering** = receding-horizon **mode-switching**(재계획마다 독립 모드). 다봉성 실재(K=32 평균-R² 회복 확증)하나 naive 샘플링이 폐루프 역효과. **v2 fix(`--flow-fixed-noise` 에피소드당 고정 x0)** 롤아웃 중 — fix가 SR 회복하는지 검정(단일테스트 ep0 여전히 timeout=불확실). 운영: v2 driver 파일 미기록 버그로 3회 미착수→재생성 해결.
- **DINOv2-avg 격리**: correct **97.5%**(전처리 confound 유의) — DINOv2 t4=92%(SigLIP2 단독 60% 정밀-3D를 DINO 기하가 해결, 콜리그 "lever=DINO geometry" 재현).

### 2026-07-16 · S2 h-flow 디코더 검증 + DINOv2-avg 격리 진단 (능동 다중에이전트)
- **S2 h-flow (사용자 제안 = cowork F5-H L3)**: 결정론 MLP h(조건평균=뭉갬)를 조건부 flow-matching 디코더로 교체(`ChunkFlowDecoder`, CFM recon/cycle, x0_std=action std; `h_mode=mlp|flow` 플래그, mlp 경로 비트동형). concat 융합에 적용.
  - **phase1 검증 통과**: K=32 **평균-R² recon +0.714/cycle +0.868 ≈ MLP(+0.749/+0.899)**, 단일샘플 −0.037/+0.119. → **p(a|ζ,z_t)에 큰 조건분산/다봉성 실재 확인**(MLP는 평균내어 뭉갬, flow는 코히런트 모드 샘플). 사용자 직관 실증. phase2 h-flow 학습 중(GPU8).
- **DINOv2-avg 격리(91.5 vs 콜리그 93-96 진단)**: 융합 공식은 코드-단 동일 → 격차 후보 = **DINOv2(그들) vs DINOv3(우리)** + eval 프로토콜(50ep/EGL vs 20ep/osmesa) + 시드. 콜리그 자체 분석 "the lever is DINOv2 geometry"(dino1only 95.2). `DualFusionAnchor`가 dino_dir로 백본 선택(dinov2=콜리그-exact 전처리, dinov3 무변경). DINOv2-avg phase1 학습 중(GPU9) → 우리 eval서 ~93-95면 격차=백본, ~91이면 격차=프로토콜.
- **patch-attention(L1+L4) 진행 정당성**: C1/C2 cross-attn 실패 = 타깃측+게이트(cold-start). 새 설계 = **관측측 무게이트 토큰+정책 attention**(S1b가 "DINOv3는 관측에 있어야 SR" 확증) = 기전적으로 다른 접근 → 진단(백본/비율) 후 착수 예정.
- 매니저: GPU 8·9 관리(NVML-loss 1회 watchdog 복구), 작업별 최적 서브에이전트(구현) + 본체 GPU 오케스트레이션.

### 2026-07-15 · S1 폐루프 양성 확정 + S1b(역할분리) 착수 (SR↔언어 tradeoff 해소 시도)
- **S1 최종(폐루프, no-aug 클린, per-task paired bootstrap 10k vs 베이스 85.0)**:
  - **avg** correct **91.5%** / wrong 17.5% / **correct−wrong +74.0pp** / paired **+6.5pp CI[+0.5,+15.0] SIG>0** — 기준①②충족.
  - **concat no-mix** correct **97.5%** / wrong 28.5% / **correct−wrong +69.0pp** / paired **+12.5pp CI[+4.5,+22.0] SIG>0(견고)** — 기준① 견고충족, ② +69<70 근소미달. per-task [100×5,95×5], 이득이 베이스 취약 공간태스크(t4 55→100·t5·t9)에 집중.
  - → **관측-레벨 융합은 강한 SR 양성**(타깃-측 C2 음성과 반대), 단 **concat은 SR↔언어 tradeoff**(SR 최고·언어 소폭 희석). avg는 균형. 방법: avg=SigLIP폴더 재현(DINOv3)/concat=cowork변형, **둘 다 cross-attn 아님** — 우리 원 cross-attn(타깃 C2)은 음성, 승부는 삽입점(관측 본류).
- **S1b 역할분리(cowork 2026-07-15f, 논문 제안 아키텍처 후보)**: 조건화 토큰(z_prev/z_cur/lang/wrist)=SigLIP2 단독(언어경로 강제) / g·h·ζ·wm=융합 concat(기하). **구현**: 조건토큰=fused[:,:1024] 슬라이스(SigLIP2-alone과 bit-exact)+zero-pad, cond_anchor flag(기존런 no-op). **+HY03 hybrid λ0.3**을 ζ의 SigLIP2 블록에만(align_block=1024 신설, info_nce가 ghat[:,:1024]↔SigLIP2 모션텍스트 정합; dz-align은 full 2048). phase1 hybrid 완료(h R² +0.744 무손실). **버그수정**: phase2/rollout DeltaAE 재구성에 align_mode/align_block 전달(hybrid ckpt logit_scale 정합). S1b-full phase2 학습 중 + S1b-noalign ablation 롤아웃 병행.
- **사전등록 S1b 판정**: SR≥91.5권 유지 AND correct−wrong≥+70 회복 AND paired CI → **논문 아키텍처 확정(role-separated fusion)**, avg/concat 폭로용 강등. 반증(SR→88.5): 이득이 조건화였음 → goal-suite.
- **S1b 결과 = 반증(완주)**: noalign correct **86.0%**(≈베이스, SR 이득 상실)·correct−wrong **+78pp**(언어 최고) / hybrid 67.5%·+60pp(HY03가 fused ζ선 역효과). → **통합 서사**: DINOv3 기하는 **관측/조건화**에 있어야 SR 기여, 타깃/코드-측(C2·S1b)은 무효. **단조 tradeoff**(조건화 DINOv3 함량↑ → SR↑·언어↓): concat 97.5/+69 → avg 91.5/+74 → S1b 86/+78. **avg = 유일 양충족 = 제안 아키텍처.** cowork에 반증 보고(`CLAUDECODE_TO_COWORK_S1b_falsified`) + goal-suite(avg 일반성) vs concat-진단 질의 → **회신 대기**(다음 파이프라인 gated).

### 2026-07-15 · F5-H S1 착수 — obs-fusion 2팔 학습 (오프라인 R² > SigLIP2 단독)
- **무엇을**: cowork `COWORK_DESIGN_F5H_final_fusion` 반영 — 관측/앵커-레벨 융합은 단순 pooled-avg 모방이 아닌 4층 설계(L1 concat no-mix+그리드토큰 / L2 융합공간 변위 Δf_t / L3 h-flow 디코더 / L4 정책 attention), 빌드순서 S1→S4 게이트. avg는 최저 베이스라인·외부 재현. **S1(융합 앵커 A/B) 착수**, no-aug 클린 레짐(감사가 aug −13pp confound 판정).
- **결과(오프라인, 폐루프는 롤아웃 판정 대기)**: **Arm-A(avg) phase1** h R² **+0.735** / **Arm-B(concat no-mix) phase1** h R² **+0.749** — 둘 다 SigLIP2 단독 0.655 초과(DINOv3 기하가 변위공간 개선 신호). **Arm-A phase2** val 0.3725(<베이스 0.3887)·액션 R² +0.679(>control 0.655)·gripper 93.8%. concat phase2 진행중.
- **어떻게**: DualFusionAnchor(avg 1024)/DualConcatAnchor(concat 2048) + lang-token zero-pad 훅(train_phase2 forward, SigLIP2 서브블록 정렬, 기존런 no-op). DINOv3 pooled 사니티 통과(실프레임 cos 0.868 < SigLIP2 0.930 = 더 분별적). **운영 사고**: avg 롤아웃 초기 드라이버가 `--tag` 미지원→cap 패턴 무효로 17롤 동시실행(GPU8 48GB near-OOM) — 조기 포착·전량 종료(concat phase2 무손상)·cap을 config명 패턴으로 교정 재launch(4캡 실작동 확인).
- **다음**: avg/concat 롤아웃 correct/wrong → 사전등록 판정(SR > no-aug SigLIP2 85.0 paired CI AND correct−wrong ≥+70pp). 양성이면 S2(h-flow) 게이트, 음성이면 관측·타깃 양삽입점 음성지도 강화.

### 2026-07-15 · cowork 융합-갭 co-sign + 재기준선 감사 (회귀 실재·태스크특이) + obs-fusion 2팔 prep
- **무엇을**: cowork `COWORK_ANALYSIS_fusion_gap`가 CC의 (A)게이트 cold-start·(B)관측융합-미검증 발견을 **전면 co-sign** + 자기정정 2건(G2 과잉일반화="반반 융합 앵커는 언어 0.5 유지 → G2 금지대상 아님" / scope서 DROP한 avg-anchor 대조팔 **철회·부활**). 고정순서: ①감사 선행 → ②obs/앵커 융합 2팔(A avg / B concat, **우리 판별하네스 correct−wrong 필수**) → ③표현력 배터리 → ④cold-start 프로브(후속).
- **감사(§3-1, 결정타 재기준선)**: no-aug 88.5 ckpt vs c2_control 80.5 ckpt를 **동일 하네스 재롤아웃**. 중간(~70%): **noaug 86.6%(밴드 복귀) vs ctrl 74.8%(회귀 재현)** → **회귀 실재**(롤아웃 노이즈 아님). **태스크-특이적**(붕괴 t8 1/8·t1·t4·t2·t5 / 정상 t3·t6·t7·t0·t9) → cowork 진단규칙상 **조립/캐시 회귀**(학습예산 아님). 유력가설: c2_control이 2-arm race 오염 aug캐시로 학습(atomic-write 픽스 이전). → 회귀 진단 에이전트 착수(캐시 무결성·조립 bit-diff, read-only/무GPU).
- **obs-fusion prep(§3-2, launch은 감사 후)**: 외부 dual_wrapper.py:75-82 공식 **verbatim 재확인**(z=L2norm((1-α)L2norm(sig)+α·L2norm(dino)), α0.5, text=SigLIP2; **외부는 DINOv2, DINOv3는 게이트403**). `DualFusionAnchor`(avg,1024,"dualfusion") + `DualConcatAnchor`(concat,2048,"dualconcat") 구현 + 4 config(phase1 재적합 2 + phase2 2). **Arm-B 블로커 발견**: train_phase2.py:299 stack이 균일폭 요구 → 2048 z vs 1024 lang 폭 불일치 = SigLIP2-서브블록 lang 토큰 hook 필요(코드 미수정, 리뷰 플래그). Arm-A(1024)는 무블로커. commit 6c70061.
- **서사(§4)**: fine-채널 음성 = "**잔차-게이트 타깃측 삽입 한정**", 관측/앵커 융합 본판정은 F-obs-fusion 2팔이 감사 후 내림 — HEADLINE 결론 정밀화(commit 4c2662e).

### 2026-07-15 · 외부 SigLIP 폴더 분석 — C2 음성 외부 재현 + 관측-레벨 융합 갭
- **무엇을**: 사용자 지시 — /SigLIP(외부 콜리그, SigLIP2+DINOv2 융합 최고) 전면 분석: 우리가 놓친 것 vs 평가 차이.
- **결과**: (1) **우리 C2 타깃-측 음성 = 외부 독립 재현** — 콜리그 M7(동일 gated DINO 잔차) 게이트스윕 95.4/94.6/94.0="이득없음~약간유해" → 우리 C2와 일치(회귀 감사와 별개로 방향 유효 가능성). (2) **진짜 갭 = 관측-레벨 융합 미검증** — 그들 승자 dual~93-95는 z_t=avg(SigLIP2,DINOv2 pooled) 관측융합(dual_wrapper.py:81, α0.5, DINOv2·v3아님, frozen), 이득 대부분 DINO 기하(t4). 우리는 타깃-측 gated만 테스트=다른 종류. (3) **평가 비차**: 50ep/det_boot/EGL/max300 vs 20ep/osmesa, large256-1024 vs so400m-1152, DINOv2 vs v3 → raw 비교 무의미(eval_vec 버그는 그들이 폐기=clean).
- **판단**: "제대로 못 만든 것"이 아니라(타깃-측은 양 팀 일치 음성), **되는 융합(관측-레벨)을 안 테스트**한 것. 공정 검정 = F-obs-fusion arm(z_t 융합+phase1 재적합, +DINOv2-only, 감사 후). cowork 회신+SCOPE 편입 제안. docs/EXTERNAL_siglip_fusion_analysis + CLAUDECODE_TO_COWORK_obs_fusion_gap.
